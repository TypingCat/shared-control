...
class TASK_PLANNER:
    ...
    def __init__(self):
        ...
        self.state = -1              # 상태: -1=휴면, 0=활동, 1=노드, 2=선택, 3=이동
        # self.count = 0
        # self.dst = Point()
    ...
    def mount_gvg(self):
        ...
        self.history = [nearest, nearest, nearest, nearest]  # 미래, 현재, 과거
    ...
    def planning(self, event):
        ...
        if len(neighbors) == 0:                         # 막다른 길일 경우,
            ...
            self.send_target(neighbors[0])              # 질문할 필요 없이 이동한다.
            self.state = 1
            # rospy.sleep(rospy.get_param('~planning_cycle', 0.5))

        elif len(neighbors) == 2:                       # 길이 두개라면,
            p0 = self.get_node(self.history[1]).point   # MI로 질문한다.
            ...
            if answer == neighbors[0]:                  # MI 서비스를 요청한다.
                self.prearrangement = [-1, neighbors[0], neighbors[1], self.history[3]]
            else:
                self.prearrangement = [-1, neighbors[1], neighbors[0], self.history[3]]
                th1, th2 = th2, th1
            ...
            rospy.Timer(rospy.Duration(rospy.get_param('~spin_cycle', 0.1)), self.take_action, oneshot=True)             # 행동을 취한다.
